...
const app = express();
const PORT = process.env.PORT || 10000; // 在 Render 上使用默認端口 10000
...
app.use(cors({
  origin: [
    'https://carparktest4frontend.vercel.app', // 前端網址
...
app.get('/', (req, res) => {
  res.json({
    message: '歡迎使用智能停車場充電系統API',
    status: '系統運行正常',
    version: '1.0.0',
    endpoints: {
      allSpots: '/api/parking-spots',
      singleSpot: '/api/parking-spot/:id',
      chargingQueue: '/api/charging-queue',
      chargingUser: '/api/charging-user',
      chargingMove: '/api/charging-move'
    },
    documentation: '使用GET方法訪問以上端點獲取數據，使用POST方法發送充電請求',
    timestamp: new Date()
  });
});
...
app.get('/api/charging-user', async (req, res) => {
  try {
    ...
    console.error('獲取充電中的用戶失敗:', error);
    res.status(500).json({
      success: false,
      message: '獲取充電中的用戶失敗',
      error: error.message
...
app.get('/api/charging-move', async (req, res) => {
  try {
    ...
    console.error('獲取充電器移動狀態失敗:', error);
    res.status(500).json({ 
      success: false,
      message: '獲取充電器移動狀態失敗',
      error: error.message 
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({
    success: false,
    message: '伺服器內部錯誤',
    error: process.env.NODE_ENV === 'production' ? '請聯繫管理員' : err.message
  });
});
